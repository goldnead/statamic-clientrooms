...
class RoomsController extends CpController
{
    ...
    public function show(int $room)
    {
        ...
        return Inertia::render('statamic-clientrooms::Rooms/Show', [
            ...
            'tasks' => $room->tasks()->get()->map(fn (ClientRoomTask $task) => $this->presentTask($task))->values()->all(),
    ...
    {
        $full = 'statamic-clientrooms::messages.source_'.$key;
        $translated = __($full);

        return is_string($translated) && $translated !== $full ? $translated : $key;
    }
